...
/**
 * 
 * Title:MultinomialModelNaiveBayes</p>
 * Description: 多项式模型朴素贝叶斯
 * 
 */
public class MultinomialModelNaiveBayes {
	...
	* 先验概率P(c)= 类c下单词总数/整个训练样本的单词总数，包含重复单词
	* @param classifier   
	* @return BigDecimal   
	* @throws
	 */
    ...
    * 由于对于每个类，(P(w_1) * P(w_2) ...P(w_n))都相同，故只比较分子
	* @param @return    
	* @return BigDecimal   
	* @throws
	 */
	public static Map<String,BigDecimal> classifyResult(Set<String> words){
		...
		for(String classifier: classifierSet){
			...
			for(String word: words){
				//求出P(w_1|C_i) * P(w_2|C_i)...P(w_n|C_i)
